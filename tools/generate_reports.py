...
def scenarios() -> list[Scenario]:
    return [
        ...
        Scenario("3", "Гость", "Регистрация клиента", "1. Нажать «Зарегистрироваться». 2. Заполнить ФИО, логин и пароль. 3. Нажать «Создать учетную запись».", "Новый логин клиента, пароль от 6 символов.", "Создан пользователь с ролью «Авторизованный клиент», открыт список товаров."),
        Scenario("4", "Авторизованный клиент", "Успешная авторизация клиента", "1. Ввести логин и пароль клиента. 2. Нажать «Войти».", "5d4zbu@tutanota.com / rwVDh9", "Открыт список товаров, в правом верхнем углу отображается ФИО клиента."),
        Scenario("5", "Авторизованный клиент", "Просмотр товаров клиентом", "1. Авторизоваться как клиент. 2. Проверить список товаров.", "Учетная запись клиента.", "Клиент видит товары без элементов поиска, фильтрации и сортировки."),
        Scenario("6", "Менеджер", "Поиск и фильтрация товаров", "1. Авторизоваться как менеджер. 2. Ввести текст поиска. 3. Выбрать производителя.", "1diph5e@tutanota.com / 8ntwUp", "Список товаров обновляется в реальном времени с учетом поиска и производителя."),
        Scenario("7", "Менеджер", "Просмотр заказов", "1. Авторизоваться как менеджер. 2. Нажать «Заказы».", "Учетная запись менеджера.", "Открыт список заказов без кнопок добавления, редактирования и удаления."),
        Scenario("8", "Администратор", "Добавление товара", "1. Авторизоваться как администратор. 2. Нажать «Добавить товар». 3. Заполнить поля. 4. Сохранить.", "94d5ous@gmail.com / uzWC67", "Товар добавлен, список обновлен, отрицательные цена и остаток недоступны."),
        Scenario("9", "Администратор", "Запрет удаления товара из заказа", "1. Авторизоваться как администратор. 2. Выбрать товар, присутствующий в заказе. 3. Нажать «Удалить товар».", "Артикул PMEZMH.", "Система показывает предупреждение и не удаляет товар."),
    ]
...
def build_screenshot_report() -> None:
    ...
    screenshots = [
        ...
        ("02_registration.png", "Рисунок 2 - Регистрация клиента"),
        ("03_guest_products.png", "Рисунок 3 - Просмотр товаров в роли гостя"),
        ("04_admin_products.png", "Рисунок 4 - Список товаров администратора"),
        ("05_orders.png", "Рисунок 5 - Список заказов"),
    ]
...
def build_flowchart_pdf() -> None:
    ...
    nodes = [
        ...
        (x, 630, 225, 42, "Отображение окна входа\nи регистрации", "process"),
        (x, 560, 225, 54, "Пользователь авторизован\nили зарегистрирован?", "decision"),
...
def build_use_case_drawio() -> None:
    ...
    cases = [
        ...
        ("u_register", "Зарегистрироваться как клиент", 250, 150),
        ("u_auth", "Авторизоваться", 250, 230),
        ("u_products", "Просматривать товары", 250, 310),
        ...
        ("u_backup", "Резервное копирование и мониторинг БД", 250, 470),
    ]
    ...
    edges = [
        ("e1", "a_guest", "u_guest"), ("e2", "a_guest", "u_register"), ("e3", "a_guest", "u_products"),
        ("e4", "a_client", "u_auth"), ("e5", "a_client", "u_products"),
        ("e6", "a_manager", "u_auth"), ("e7", "a_manager", "u_products"),
        ("e8", "a_manager", "u_filter"), ("e9", "a_manager", "u_orders"),
        ("e10", "a_admin", "u_auth"), ("e11", "a_admin", "u_products"),
        ("e12", "a_admin", "u_filter"), ("e13", "a_admin", "u_orders"),
        ("e14", "a_admin", "u_product_crud"), ("e15", "a_admin", "u_order_crud"),
        ("e16", "a_admin", "u_backup"),
    ]
    for edge in edges:
...
def build_use_case_pdf() -> None:
    ...
    use_cases = [
        ("Войти как гость", 230, 440),
        ("Зарегистрироваться", 230, 370),
        ("Авторизоваться", 230, 300),
        ("Просматривать товары", 230, 230),
    ...
    # Associations.
    links = [
        ((60, 370), (230, 464)), ((60, 370), (230, 394)), ((60, 370), (230, 254)),
        ((60, 230), (230, 324)), ((60, 230), (230, 254)),
